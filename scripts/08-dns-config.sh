...
if [ -n "${RANCHER_DOMAIN:-}" ]; then
  ...
  if [ "$DNS_CONFIGURED" = false ] && [ -n "$RANCHER_LB_IP" ]; then
    echo "🔧 Para configurar DNS correctamente, necesitas configurar TODOS estos subdominios:"
    echo ""
    echo "   OPCIÓN 1 - DNS Server (Recomendado para producción):"
    echo "   ====================================================="
    echo "   Agrega estos registros A en tu servidor DNS:"
    echo "   ${K8S_API_DOMAIN}.     IN    A    ${LB_IP}"
    echo "   ${K8S_REG_DOMAIN}.     IN    A    ${LB_IP}"
    echo "   ${RANCHER_DOMAIN}.     IN    A    ${LB_IP}"
    echo ""
    echo "   OPCIÓN 2 - /etc/hosts (Solo para pruebas):"
    echo "   =========================================="
    echo "   Agrega estas líneas a /etc/hosts en máquinas cliente:"
    echo "   ${LB_IP} ${K8S_API_DOMAIN}"
    echo "   ${LB_IP} ${K8S_REG_DOMAIN}"
    echo "   ${LB_IP} ${RANCHER_DOMAIN}"
    echo ""
    echo "   OPCIÓN 3 - DNS Externo (Si usas proveedor DNS):"
    echo "   ============================================="
    echo "   Configura estos registros A en tu proveedor DNS:"
    echo "   Host: $(echo "$K8S_API_DOMAIN" | cut -d'.' -f1)    Tipo: A    Valor: ${LB_IP}"
    echo "   Host: $(echo "$K8S_REG_DOMAIN" | cut -d'.' -f1)    Tipo: A    Valor: ${LB_IP}"
    echo "   Host: $(echo "$RANCHER_DOMAIN" | cut -d'.' -f1)    Tipo: A    Valor: ${LB_IP}"
    echo "   TTL: 300"
    echo ""
